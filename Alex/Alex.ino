...
void dbprintf(char *format, ...) { //debug printing
  va_list args;
  char buffer[128];
...
void forward(float dist, float speed)
{
  if(dist > 0)
    ...
    deltaDist = 9999999; //dist = 0, large no. so Alex moves indefinitely
...
// needed to turn an angkle
unsigned long computeDeltaTicks(float ang)
{
  // We assume that angularmoved = linear siatance moved in one wheel
